...
- 記憶體：16GB RAM
- 硬碟：20GB SSD
- GPU：NVIDIA GTX 1060 或更高（可選，用於加速AI運算）

### Q2: 系統可以預測多遠的未來？

系統可以預測未來60分鐘的交通狀況，每5分鐘更新一次預測。短期預測（5-15分鐘）最準確，長期預測（30-60分鐘）提供參考價值。

### Q3: 為什麼需要17種基準模型？

這17種模型（DCRNN、LSTM、ARIMA等）僅用於研究對比和性能驗證，證明我們的MT-STNet模型表現最佳。實際系統運行時，只使用MT-STNet、圖卷積神經網路和地震波物理模型進行預測。

### Q4: AI助手的數據會上傳到雲端嗎？

不會。我們使用Ollama在本地運行大型語言模型，所有對話和數據處理都在您的電腦上完成，不會上傳到任何外部伺服器，完全保護您的隱私。

### Q5: 系統支援哪些地區？

目前系統專注於台灣高速公路（國道1號、3號等），數據來自交通部TDX平台。若要擴展到其他地區，需要該地區的即時交通數據API。

### Q6: 如何提高預測準確度？

預測準確度會隨著累積更多歷史數據而提升。建議：
- 持續運行數據收集系統
- 定期重新訓練模型
- 回報系統預測錯誤的案例
- 參與模型改進研究
...
本專案採用MIT授權，可以自由用於商業用途。但請注意：
- 遵守TDX平台的使用條款
- 遵守Google Maps API的授權限制
- 標註原始專案來源

### Q8: 遇到問題如何求助？

- **GitHub Issues**：回報bug或功能請求
- **GitHub Discussions**：技術討論與經驗分享
- **專案文檔**：查閱詳細的技術說明
