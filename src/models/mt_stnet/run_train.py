...
from core.st_block import STAttBlock, BridgeTrans, MT_STNet

tf.reset_default_graph()
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"
logs_path = "board"
os.environ['CUDA_VISIBLE_DEVICES'] = ''  # Use CPU on Mac
...
class Model(object):
    ...
    def adjecent(self):
        ...
        for line in data[['src_FID', 'nbr_FID']].values:
            ...
            src_idx = line[0] - 1
            nbr_idx = line[1] - 1
            if 0 <= src_idx < self.hp.site_num and 0 <= nbr_idx < self.hp.site_num:
                adj[src_idx][nbr_idx] = 1
        return adj
...
def main(argv=None):
    ...
    print('請輸入一個數字：1 或 0 (1代表訓練模式，0代表測試模式)')
    val = input('您的選擇：')  # 讓使用者輸入選擇
    
    # 如果使用者沒有輸入或輸入無效，預設為訓練模式
    if val not in ['0', '1']:
        val = '1'
        print('輸入無效，自動選擇訓練模式 (1)')
